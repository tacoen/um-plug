...
/*	
function um_layouts_bind() {
	$layout = um_getoption('layout','umt');
	if ($layout != '') {
		?><style type="text/css" id="um_layout_preview"><?php echo join('',file(get_stylesheet_directory()."/layouts/$layout")) ?></style><?php
	}
}

function um_layouts_init() {
	add_action('wp_head','um_layouts_bind');
}
